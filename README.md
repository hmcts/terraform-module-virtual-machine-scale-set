...
| <a name="input_network_interfaces"></a> [network\_interfaces](#input\_network\_interfaces) | One or more network\_interface can be set here, at least one of the network\_interface have to be primary | <pre>map(object({<br>    subnet_id = string<br>  }))</pre> | n/a | yes |
